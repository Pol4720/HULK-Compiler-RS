...
use crate::{
    hulk_tokens::{
        BinaryExpr, Block, BooleanLiteral, DestructiveAssignment, ForExpr, FunctionCall,
        FunctionDef, HulkFunctionInfo, Identifier, IfExpr, LetIn, NumberLiteral, ProgramNode,
        StringLiteral, UnaryExpr, WhileLoop,
    },
    typings::{
        types_ast::{HulkTypesInfo, TypeAST},
        types_node::TypeNode,
    },
...
impl Visitor<TypeNode> for SemanticVisitor {
    ...
    fn visit_destructive_assignment(&mut self, node: &DestructiveAssignment) -> TypeNode {
        if self.current_scope.variables.contains_key(&node.identifier) {
            ...
            self.current_scope
                .variables
                .insert(node.identifier.clone(), new_type.clone());
            new_type
        } else {
    ...
    fn visit_function_def(&mut self, node: &FunctionDef) -> TypeNode {
        ...
        for param in &node.params {
            if let Some(param_type) = self.type_ast.get_type(&param.signature) {
                self.current_scope
                    .variables
                    .insert(param.name.clone(), param_type.borrow().clone());
                arg_types.push(param_type.borrow().clone());
            } else {
                self.new_error(SemanticError::UndefinedType(param.signature.clone()));
                self.current_scope
                    .variables
                    .insert(param.name.clone(), self.get_type(&HulkTypesInfo::Unknown));
            ...
            .contains_key(&node.name)
        {
            ...
            self.current_scope.declared_functions.insert(
                ...
                HulkFunctionInfo::new(
                    ...
                    arg_types.clone(),
                    self.type_ast
                        .get_type(&node.return_type.clone())
                        .unwrap()
                        .borrow()
                        .clone(),
                ),
        ...
        self.current_scope.declared_functions.insert(
            node.name.clone(),
            HulkFunctionInfo::new(node.name.clone(), arg_types, return_type_node.clone()),
        );
        return_type_node
    }
    ...
    fn visit_if_else(&mut self, node: &IfExpr) -> TypeNode {
        ...
        if then_type != else_type {
            let lca_option = self
                .type_ast
                .find_lca(then_type.type_name.as_str(), else_type.type_name.as_str());
